// Ampere (SM80/SM86) fp8-STORAGE SCALED GEMM (port of gemm/fp8_h100_scaled):
// D[m,n] = scale_a[m] * scale_b[n] * sum_k A[m,k]*B[n,k], fp8e4m3 inputs, fp32 out.
// Derived from fp8_ampere_gemm.cu -- the Marlin approach: Ampere has no fp8
...
//
// Build:
//   /usr/local/cuda/bin/nvcc fp8_ampere_gemm_scaled.cu -std=c++20 --extended-lambda \
//     --expt-relaxed-constexpr -I../../../include -I.. -DKITTENS_SM86 \
//     -gencode arch=compute_86,code=sm_86 -lcuda -lcudart -O2 -o fp8_ampere_gemm_scaled.out
...
struct globals {
    ...
    b_gl b;
    d_gl d;
    const float *scale_a; // per output row (M)
    const float *scale_b; // per output col (N)
...
__global__ void kernel(const __grid_constant__ globals<C> g) {
    ...
    if (warpgroup_id == C::NUM_CONSUMERS) {
        ...
        for (int task_id = blockIdx.x; task_id < num_blks; task_id += gridDim.x) {
            ...
            { // apply per-row/per-col scales to the accumulator (fragment addressing:
              // data[k] rows +8*(k%2), cols +8*(k/2); lane owns (lane/4, (lane%4)*2))
                int2 tile_coord = get_swizzled_2d_idx<C::SUPERGROUP_SIZE>(rblks, cblks, task_id);
                const int row_base = (tile_coord.x*2 + warpgroup_id)*(C::Mb/2) + warp_row0;
                const int col_base = tile_coord.y*C::Nb;
                const int lane = kittens::laneid();
                const float sa[2] = { g.scale_a[row_base + lane/4], g.scale_a[row_base + lane/4 + 8] };
                #pragma unroll
                for (int j = 0; j < C::Nb/16; j++) {
                    #pragma unroll
                    for (int k = 0; k < 4; k++) {
                        const int col = col_base + j*16 + (lane%4)*2 + (k/2)*8;
                        d_reg.tiles[0][j].data[k].x *= sa[k%2] * g.scale_b[col];
                        d_reg.tiles[0][j].data[k].y *= sa[k%2] * g.scale_b[col+1];
                    }
                }
            }
...
__global__ void reference_fp8_gemm_kernel(float* D, const int8* A, const int8* B,
                                          const float* sa, const float* sb, int M, int N, int K) {
    ...
    if (row < M && col < N) {
        ...
        D[row * N + col] = acc * sa[row] * sb[col];
    }
}
static inline void reference_fp8_gemm(float* D, const int8* A, const int8* B,
                                      const float* sa, const float* sb, int M, int N, int K) {
    dim3 block(16, 16);
    dim3 grid((N + 15) / 16, (M + 15) / 16);
    reference_fp8_gemm_kernel<<<grid, block>>>(D, A, B, sa, sb, M, N, K);
}
...
__host__ double run_benchmark(size_t M, size_t N, size_t K, bool ncu = false) {
    ...
    CUDACHECK(cudaMalloc(&d_scale_a, M*sizeof(float)));
    CUDACHECK(cudaMalloc(&d_scale_b, N*sizeof(float)));
    fill<float, FillMode::RANDOM>(d_scale_a, M, 7, 0.5f, 2.0f);
    fill<float, FillMode::RANDOM>(d_scale_b, N, 8, 0.5f, 2.0f);
    ...
    // Compute reference GEMM on device (fp8 in, fp32 accumulate/out, scaled)
    reference_fp8_gemm(d_D_ref, d_A[0], d_B[0], d_scale_a, d_scale_b, M, N, K);
    CUDACHECK(cudaDeviceSynchronize());
    ...
    for (int i = 0; i < arg_group_count; i++) {
        ...
        g.push_back(globals<C>{Ag, Bg, Dg, d_scale_a, d_scale_b});
    }
